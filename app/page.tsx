...
export default function Home() {
  ...
  async function handleGenerate() {
    ...
    try {
      // Minimum 1.8s so the loader feels intentional
...
function HomeScreen({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute top-[12%] left-[8%] font-display text-4xl text-sand/40 select-none"
      ...
      <motion.div
        className="absolute top-[18%] right-[10%] text-5xl text-rose/50 select-none"
        animate={{ y: [0, 10, 0], rotate: [5, -5, 5] }}
      ...
      <motion.div
        className="absolute bottom-[15%] left-[12%] text-4xl text-sage/50 select-none"
        animate={{ y: [0, -8, 0] }}
      ...
      <motion.div
        className="absolute bottom-[22%] right-[14%] font-display text-5xl text-sunset/40 select-none"
      ...
      >
        <div className="font-mono text-[10px] tracking-[4px] uppercase text-sepia mb-6">
          · est. 2026 · toronto ·
        </div>
        ...
        <div className="flex items-center justify-center gap-3 my-5">
          ...
          <span className="font-italic italic text-sepia text-lg">
            tiny memories, soft &amp; slow
          </span>
          <div className="w-10 h-px bg-sand" />
        </div>

        <p className="font-italic italic text-sepia text-base mb-10 max-w-xs mx-auto leading-relaxed">
          turn your favorite photos into vintage film strips. free, instant,
          no signup.
        </p>
        ...
        <div className="font-mono text-[10px] tracking-[3px] uppercase text-faded mt-10">
          — up to 4 photos —
        </div>
      </motion.div>
    </motion.section>
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-8">
        <button
          onClick={onBack}
          className="font-mono text-xs tracking-widest text-sepia hover:text-ink transition-colors"
        >
          ← back
        </button>
        <div className="text-right">
          <div className="font-mono text-[10px] tracking-[3px] uppercase text-sepia">
            step 02
          </div>
      ...
      <div className="tape paper-card rounded-sm p-5 md:p-6 mb-6">
        <div className="flex justify-between items-end mb-4">
          ...
          {photos.length > 1 && (
            <button
              onClick={onShuffle}
              className="font-mono text-[10px] tracking-widest uppercase bg-sunset/20 text-ink px-3 py-2 rounded-sm hover:bg-sunset/40 transition-colors border border-sunset/40"
            >
              ↻ shuffle
            </button>
          )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              {photos.length < 4 && (
                ...
                >
                  ...
                  <div className="font-mono text-[9px] tracking-widest uppercase text-sepia">
                    add photo
                  </div>
                </button>
              )}
      ...
      <motion.button
        ...
        className={`w-full py-5 rounded-sm font-mono text-sm tracking-[4px] uppercase font-bold transition-all ${
          photos.length === 0
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-8">
        <button
          onClick={onBack}
          className="font-mono text-xs tracking-widest text-sepia hover:text-ink"
        >
          ← edit
        </button>
        <div className="font-mono text-[10px] tracking-[3px] uppercase text-sepia">
      ...
      {/* Result image */}
      <motion.div
        initial={{ rotate: -8, y: -100, opacity: 0 }}
      ...
      >
        {/* Tape strips */}
        <div
          className="absolute -top-2 left-8 w-20 h-6 bg-yellow-100/80 border border-yellow-200/50 z-10"
          style={{ transform: "rotate(-12deg)" }}
        />
        <div
          className="absolute -top-2 right-8 w-20 h-6 bg-yellow-100/80 border border-yellow-200/50 z-10"
